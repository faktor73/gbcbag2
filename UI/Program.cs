...
#if EASTPIXEL_BUILD
		public static string ExePath => Process.GetCurrentProcess().MainModule?.FileName ?? Path.Join(Path.GetDirectoryName(AppContext.BaseDirectory), "eastpixel.exe");
		public static string DLLPath => Path.Join(Path.GetDirectoryName(AppContext.BaseDirectory), "game/game.gb");
#else
